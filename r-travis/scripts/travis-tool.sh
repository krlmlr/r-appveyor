...
EnsureRemotes() {
    ...
    if ! Rscript -e 'if (!("remotes" %in% rownames(installed.packages()))) q(status=1)' ; then
        ...
        Rscript -e 'path <- file.path(tempdir(), "remotes_1.0.0.tar.gz"); download.file("http://cran.r-project.org/src/contrib/remotes_1.0.0.tar.gz", path); install.packages(path, repos = NULL)'
    fi
}
